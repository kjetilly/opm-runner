...
class SampleRunner:
    def __init__(self, *, submitter, script, inputfile, flowpath, args_to_flow, dry_run, sampledir):
    ...
    def run_for_sample(self, sampleid):
        arguments_to_sample = [
            sys.executable, 
            self._script, 
            "--inputfile", self._inputfile,
            "--outputdir", self._sampledir(sampleid),
            "--parametersfile", os.path.join(self._sampledir(sampleid), 'parameters.txt'),
            '--flowpath', self._flowpath
        ]
        if self._dry_run:
        ...
        self._submitter(arguments_to_sample, check=True, stderr=stderrfile, stdout=stdoutfile)
